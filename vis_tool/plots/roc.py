...
def plot_roc_curves(
    ...
    title: str = "ROC Curves (one-vs-rest)", save_path: Optional[str] = None,
    figsize: tuple[float, float] = (8, 6),
) -> plt.Figure:
    """绘制多分类 one-vs-rest ROC 曲线。"""
    ...
    for index in range(num_classes):
        ...
        if targets.min() == targets.max():
            continue
        fpr, tpr, _ = roc_curve(targets, y_score[:, index])
        ax.plot(fpr, tpr, color=get_cmap(num_classes, categorical=True)[index], lw=2,
                label=f"{class_names[index]} (AUC={auc(fpr, tpr):.3f})")
    ax.plot([0, 1], [0, 1], "k--", lw=1, alpha=0.4, label="Random Classifier")
    ax.set(xlim=(-0.02, 1.02), ylim=(-0.02, 1.02), xlabel="False Positive Rate", ylabel="True Positive Rate", title=title)
