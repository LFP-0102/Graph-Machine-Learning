...
在 Cora 数据集上比较三种模型的 transductive 节点分类性能。
"""
import torch
...
if __name__ == "__main__":
    ...
    # ── GraphSAGE ────────────────────────────────────────────
    sage = GraphSAGE(input_dim=data.num_features, hidden_dim=16,
                     output_dim=data.num_classes, dropout=0.5)
    sage_acc = run_experiment(sage, data.edge_index, data, seed=42)
    print(f"GraphSAGE  Test Acc: {sage_acc:.4f}")
    ...
    print(f"\n{'='*50}")
    print(f"GCN: {gcn_acc:.4f}  |  GraphSAGE: {sage_acc:.4f}  |  GAT: {gat_acc:.4f}")
